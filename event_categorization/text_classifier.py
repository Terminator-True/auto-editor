    ...
    def classify(self, description: str, tokens: Optional[List[str]] = None) -> Dict:
        ...
        try:
            if self.model is not None:
                # prefer model.classify or model.generate
                if hasattr(self.model, 'classify'):
                    llm_used = self.model.classify(description)
                elif hasattr(self.model, 'generate'):
                    llm_used = self.model.generate(description)
            else:
                # try compatibility shim
                wrapper = pipeline.VisionLLMWrapper()
                if wrapper and hasattr(wrapper, 'analyze_frame'):
                    # shim cannot classify text, so skip
                    llm_used = None
        ...
        if llm_used:
            # accept many shapes: dict or tuple
